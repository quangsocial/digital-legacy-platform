...
function PreCheckoutPageContent() {
    ...
    const handleSubmit = async (e: any) => {
        ...
        try {
            // Gọi API tạo order: chỉ truyền email, product_variant_id, amount (total), customer_name
            const res = await fetch('/api/admin/orders/create', {
                ...
                body: JSON.stringify({
                    ...
                    amount: price, // lấy từ tổng giá trị thực tế
                    customer_name: form.name,
                })
            });
            const data = await res.json();
            if (!res.ok || !data.order?.order_number) {
                setError(data.error || 'Không tạo được đơn hàng');
                return;
            }
            // Remove dashes from order_number before redirect (defensive)
            const orderNumber = (data.order.order_number || '').replace(/-/g, '');
            router.push(`/product/checkout?orderNumber=${orderNumber}&method=${form.method}`);
        } catch (err) {
